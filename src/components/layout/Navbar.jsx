...
function ProfileDropdown({ user, userRole, avatarUrl, onSignOut }) {
  ...
  useEffect(() => setCustomAvatar(avatarUrl), [avatarUrl])

  // Reflect a Settings-page avatar save immediately, without a page refresh.
  useEffect(() => {
    const handler = (e) => setCustomAvatar(e.detail?.avatarUrl ?? null)
    window.addEventListener(AVATAR_UPDATED_EVENT, handler)
    return () => window.removeEventListener(AVATAR_UPDATED_EVENT, handler)
  }, [])

  const meta = user?.user_metadata || {}
  // A custom upload wins over the OAuth-provider photo.
  const avatar = customAvatar || meta.avatar_url || meta.picture || null
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Avatar src={avatar} name={fullName} size="nav" className="ring-2 ring-white/70" />
        <svg
...
export default function Navbar() {
  ...
  // Fetch role + avatar from user_profiles after getting the auth user.
        ...
        <div className="px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center gap-3 shrink-0">
              {user ? (
                <ProfileDropdown user={user} userRole={userRole} avatarUrl={avatarUrl} onSignOut={handleSignOut} />
              ) : (
                <>
                  <Link
