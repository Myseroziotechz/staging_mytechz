...
import { SelectInput } from '@/app/(app)/profile/components/shared/Fields'
import Switch from '@/components/ui/Switch'
import useInstantPreferences from '../hooks/useInstantPreferences'

const FREQUENCIES = [
  { value: 'instant', label: 'Instant — as soon as a job is posted' },
  ...
  return (
    <SectionCard title="Notifications" icon={ICON} canEdit={false} error={error}>
      ...
      <div className="divide-y divide-gray-100 mb-4">
        ...
        />
        {settings.notify_job_alerts && (
          <div className="py-3">
            <SelectInput
              label="Alert frequency"
              hint="How often job alert emails are sent — in-app notifications are always instant."
              options={FREQUENCIES}
              value={settings.notify_frequency}
              onChange={(e) => update('notify_frequency', e.target.value)}
            />
          </div>
        )}
        <Switch
