...
export async function createJobAction(payload) {
  ...
  // A verified recruiter's post is live immediately (status === 'active') —
  // fan out job-alert notifications after the response is sent, so this
  // never slows down the recruiter's own request.
  if (status === 'active' && data?.id) {
    after(() => dispatchJobNotifications(data.id))
  }
