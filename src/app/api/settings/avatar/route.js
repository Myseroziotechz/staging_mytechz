...
export async function POST(request) {
  ...
  try {
    ...
    // Best-effort cleanup of the previous file(s) so replacing a photo doesn't
    // leave orphaned objects in Storage forever. Never blocks or fails the
    // save — the new URL is already committed above the point this can fail.
    try {
      const { data: files } = await supabase.storage.from('avatars').list(user.id)
      const stale = (files || []).filter((f) => `${user.id}/${f.name}` !== path)
      if (stale.length) {
        await supabase.storage.from('avatars').remove(stale.map((f) => `${user.id}/${f.name}`))
      }
    } catch (cleanupErr) {
      console.error('[settings/avatar] stale file cleanup failed:', cleanupErr)
    }
