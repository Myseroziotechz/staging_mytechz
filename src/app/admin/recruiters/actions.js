...
export async function rejectRecruiter(_prev, formData) {
  const userId = str(formData, 'user_id')
  // Captured for a future "reason for rejection" UI/column — neither exists
  // yet (no caller passes this field, no recruiter_profiles column to store
  // it), so it isn't persisted. Kept named/prefixed rather than dropped so
  // it's visible as unfinished, matching this file's `_prev` convention for
  // intentionally-unused bindings.
  const _reason = str(formData, 'reason')
  if (!userId) return { error: 'Missing user ID' }
